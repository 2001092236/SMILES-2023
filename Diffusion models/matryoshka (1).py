...
class Texture(nn.Module):
    ...
    def forward(self, points):
        ...
        matryoshka_colors = F.grid_sample(
        ...
        matryoshka_colors = matryoshka_colors.view(4, -1)[:3, :].T.view(*points.shape)
...
"""
def get_pixel_depths(points, camera_origins):
    return torch.norm(points - camera_origins[:, None, None, :], dim=-1)

def get_pixel_colors(points):
    light_source = 3. * torch.as_tensor([1.0, 0.0, 1.0]).to(points)
    albedo = torch.as_tensor([1.0, 1.0, 1.0]).to(points)
    normals = calc_normals(points)
    light_dir = normalize(light_source - points, dim=-1)
    shadow_coefficient = (normals * light_dir).sum(-1, keepdim=True).clamp(0.1)

    light_rays = {'origins': points,
                  'directions': normalize(light_source - points, dim=-1)}
    in_shadow = cast_soft_shadow(light_rays)
    return albedo * shadow_coefficient * in_shadow

def get_random_pixel_colors(points):
    f = RandomFunction(32768).to(points)
    light_source = 3. * torch.as_tensor([1.0, 0.0, 1.0]).to(points)
    albedo = torch.sigmoid(f(points))
    normals = calc_normals(points)
    light_dir = normalize(light_source - points, dim=-1)
    shadow_coefficient = (normals * light_dir).sum(-1, keepdim=True).clamp(0.1)
